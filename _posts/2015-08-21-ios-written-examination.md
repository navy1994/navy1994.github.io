...
@property是一个属性访问声明，扩号内支持以下几个属性： 1，getter=getterName，setter=setterName，设置setter与getter的方法名 2，readwrite,readonly，设置可供访问级别 

3，assign，setter方法直接赋值，不进行任何retain操作，为了解决原类型与环循引用问题 4，retain，setter方法对参数进行release旧值再retain新值，所有实现都是这个顺序(CC上有相关资料) 
